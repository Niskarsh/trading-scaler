...
export default function SymbolSearch({ segment, searchQuery, onSearchChange, onSelect }: { segment: string, searchQuery: string, onSearchChange: (query: string) => void, onSelect: (sym: string, id: string, ts: number) => void }) {
    ...
    <div className="relative">
      ...
      {results.length > 0 && (
        <div className="absolute z-50 w-full bg-[#0d1117] border border-[#30363d] mt-1 rounded-xl shadow-2xl">
          {results.map(s => (
            <div key={s.id} onClick={() => handleSelect(s.symbol, s.id, s.tickSize)} className="p-4 hover:bg-[#21262d] border-b border-[#30363d] last:border-0 flex justify-between items-center">
